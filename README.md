...
1. **看变更**：右栏「Diff 评审」tab 列出相对评审基线（存储的 marker，缺省主线预选）的全部变更 —— 含已提交 +
   未提交 + 未跟踪（各有可见标记），改名显示 `old → new`（各视图都保留；树上表达不了的评审信息），
   可切树形/平铺、仅变更/全树。**文件名下的所在目录副行只在平铺清单展示**（R26）—— 树形/全树的目录结构
   本身已表达路径，不再重复。**工具条上方有文件筛选框**
   （R24）：输入即筛（大小写不敏感的路径子串，rename 旧路径也参与命中），树形/平铺、仅变更/全树共用；筛选时
   折叠的目录自动展开，无匹配给空态文案，清除按钮一键还原；纯客户端过滤，不新增请求。**全树里未变更的文件也
   可以点击**（R18，降调样式）：打开它的**全文预览**（基线内容，行号与 HEAD 一致），行级/文件级评论照常可写 ——
   给还没改的文件提前留评审意见（如「这个文件拆一下」）。**未跟踪文件同样可以点**（R22）：打开它的**工作区
   内容全文预览**（git 里还没有它，预览的就是盘上内容），行级/文件级评论照常可写；预览不可得（旧版服务端/
   文件已消失）时回落「还没有 diff」提示。
2. **选基线**：头部「更改」打开基线选择器（本地/远程分支、最近提交、手动输入）；覆盖只影响本次评审，**永不移动存储的 marker**。
3. **读 diff**：点文件 → diff 面板（旧/新双行号、可展开的未变更折叠、新文件/删除/改名/二进制/截断各态；
   未跟踪文件与未变更文件一样走全文预览态，头部注记说明内容来源）。**Markdown 文件可切「渲染」
   视图**（R23，头部开关，预览态与变更态都可用）：零依赖 Markdown 渲染器（标题/列表/表格/代码块/引用/
   任务清单，全部转义 + 链接 scheme 白名单，无 innerHTML）；变更文件渲染的是**当前内容**（新侧全文按需
   懒取，缓存随刷新作废重取）；渲染态只读，行级评论与选中在原文视图可用。
...
6. **当唯一文件浏览器用**（R25）：本 tab 自带全树浏览（仅变更↔全树开关、未变更文件点击预览、筛选框），可以把
   宿主自带的「文件列表」tab 收起来、只用这一个 —— **设置 → 界面布局 → 右侧面板**里取消勾选「文件列表」即可
   （隐藏的入口仍能从顶栏溢出菜单进，随时勾回来）。与宿主文件树对齐的行级动作在文件行**悬停浮现**：
   - **引**：把文件以引用方式放进输入框（reference 附件，AI 按需读取）—— 宿主树的「仅引用路径」同款形态；
   - **附**：把全文内联附进输入框（`/blob?side=new` 拉工作树/HEAD 全文，单文件 64KiB 上限；超限/二进制/
     截断/拉取失败自动降级为引用，绝不内联半截文本）—— 宿主树的「附加内容到对话」对应；
   - **复**：复制绝对路径（仓库根取自 refresh 已有的 `/marker` 响应）。动作点击不触发行的选中/打开；
     通道全是宿主桥 `compose({text, attachments})` 受支持契约（只填草稿，绝不发送）。
   动作芯片复刻宿主文件树的 file-attach 设计（22×22 彩色软底、同款 Feather 图标 FiLink/FiPlus/FiClipboard、
   同款 tooltip 原句「仅引用路径（AI 按需读取）/附加内容到对话/复制路径」、data-tip 纯 CSS 气泡、悬停实底反转；
   复制成功换勾图标 + 闪绿同宿主 copied 态）；悬停文件行时 +/− 计数列隐藏、动作芯片浮现（键盘聚焦同样触发）。
   已知取舍：文件管理操作（剪切/粘贴/重命名）、下载、图片缩放预览、多工作区根切换不在此 tab 范围内
   （评审浏览器定位）；待办 widget 锚在宿主文件树下，收起该 tab 后其去留以宿主行为为准。
